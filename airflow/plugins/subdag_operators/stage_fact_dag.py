...
def stage_fact_s3_to_redshift(
    ...
    check_stl_branch = STLCheckOperator(
        task_id=f'stl_check_{staging_table}',
    ...
    staging_success_task = PythonOperator(
        task_id=f'staging_success_check_{staging_table}',
        python_callable=staging_success_check,
        op_kwargs={'redshift_conn_id': redshift_conn_id, 'staging_table': staging_table, 'error_table': error_table},
        dag=dag,
        provide_context=True
    )
